...
Public Class SalesOrder
        ...
        Try
            ...
            Else
                ...
                If blnFlag = "Success" Then
                    ComVar.StrQuery = "Select ""U_DocEntry"",""U_ItemCode"",""U_ItemName"",""U_Quantity"",""U_frgName"",""U_UoMName"",""U_Price"",""U_LineNodays"",Cast(""U_ShpDate"" as varchar(11)) as ""U_ShpDate"" from " & DBName & ".""U_ORDR"" where ""U_SessionId""='" & Session("SessionId").ToString() & "' order by ""U_DocEntry"""
    ...
    Private Sub grdSalesItem_PageIndexChanging(ByVal sender As Object, ByVal e As GridViewPageEventArgs) Handles grdSalesItem.PageIndexChanging
        ...
        ComVar.StrQuery = "Select ""U_DocEntry"",""U_ItemCode"",""U_ItemName"",""U_Quantity"",""U_frgName"",""U_UoMName"",Cast(""U_ShpDate"" as varchar(11)) as ""U_ShpDate"",""U_Price"",""U_LineNodays"" from " & DBName & ".""U_ORDR"" where ""U_SessionId""='" & Session("SessionId").ToString() & "' order by ""U_DocEntry"""
        ObjDA.GridviewBind(ComVar.StrQuery.ToUpper(), grdSalesItem)
    End Sub
    ...
    Private Sub grdSalesItem_RowDeleting(ByVal sender As Object, ByVal e As GridViewDeleteEventArgs) Handles grdSalesItem.RowDeleting
        Try
            ...
            ComVar.StrQuery = "Select ""U_DocEntry"",""U_ItemCode"",""U_ItemName"",""U_Quantity"",""U_UoMName"",""U_frgName"",Cast(""U_ShpDate"" as varchar(11)) as ""U_ShpDate"",""U_Price"",""U_LineNodays"" from " & DBName & ".""U_ORDR"" where ""U_SessionId""='" & Session("SessionId").ToString() & "' order by ""U_DocEntry"""
            ObjDA.GridviewBind(ComVar.StrQuery.ToUpper(), grdSalesItem)
        Catch ex As Exception
            DBConnectionDA.WriteError(ex.Message)
            ComVar.StrMsg = "alert('" & ex.Message & "')"
    ...
    Protected Sub btngoItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btngoItem.Click
        If txtsearchItem.Text.Trim() <> "" Then
            ObjDA.GridviewBind("SELECT T1.""ItemCode"", T1.""ItemName"" FROM " & DBName & ".OSCN T0  INNER JOIN " & DBName & ".OITM T1 ON T0.""ItemCode"" = T1.""ItemCode"" WHERE T0.""CardCode"" ='" & Session("UserCode").ToString() & "' and T1.""ItemCode""  like '%" + txtsearchItem.Text.Trim().Replace("'", "''") + "%' order by ""DocEntry"" desc", grdItems)
        ElseIf txtsearchItemNa.Text.Trim() <> "" Then
            ObjDA.GridviewBind("SELECT T1.""ItemCode"", T1.""ItemName"" FROM " & DBName & ".OSCN T0  INNER JOIN " & DBName & ".OITM T1 ON T0.""ItemCode"" = T1.""ItemCode"" WHERE T0.""CardCode"" ='" & Session("UserCode").ToString() & "' and T1.""ItemName""  like '%" + txtsearchItemNa.Text.Trim().Replace("'", "''") + "%' order by ""DocEntry"" desc", grdItems)
        Else
            ObjDA.GridviewBind("SELECT T1.""ItemCode"", T1.""ItemName"" FROM " & DBName & ".OSCN T0  INNER JOIN " & DBName & ".OITM T1 ON T0.""ItemCode"" = T1.""ItemCode"" WHERE T0.""CardCode"" ='" & Session("UserCode").ToString() & "'", grdItems)
        End If
        popemployee.Show()
